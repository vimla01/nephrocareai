...
# falls back to a local dev db if no env var is set, so the api still boots without a .env
...
def get_conn():
    ...
    # every function in this module opens its own short-lived connection rather than pooling -
    # simplest thing that works for a small local api, not meant to scale to many concurrent users
    try:
        return psycopg2.connect(DATABASE_URL)
    except psycopg2.OperationalError as e:
        err_str = str(e)
        if "does not exist" in err_str or "3D000" in err_str:
            # target database is missing, try to create it on the fly instead of failing hard
            try:
                ...
                temp_conn.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)  # CREATE DATABASE can't run inside a transaction
        ...
        # last resort: try the local peer/default auth connection before giving up
...
def init_db() -> None:
    ...
    # every table is prefixed nephrocare_ so this app can share a postgres instance/db with other projects
    queries = [
        """
    ...
    # each history table (predictions/scans/symptoms/food checks) just stores a jsonb blob per
    # event instead of its own columns, since the shape of each entry varies by feature
    conn = get_conn()
    try:
...
def get_user_by_email(email: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            # case insensitive match since emails get stored lowercased but old rows may not be
            ...
            row = cur.fetchone()
            if row:
                row['created_at'] = row['created_at'].timestamp()  # datetime isn't json serializable
                return dict(row)
            return None
    finally:
        conn.close()
...
def create_user(user_id: str, name: str, email: str, password_hash: str, oauth_provider: Optional[str] = None) -> Dict[str, Any]:
    # user_id is generated by the caller (auth.py), not a db sequence
    conn = get_conn()
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
...
def get_profile(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            # column aliases here are camelCase to match what the frontend expects directly, no remapping needed
            cur.execute(
                """
...
def upsert_profile(user_id: str, profile_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            # one profile row per user, so insert-or-update in a single query instead of a separate exists check
            cur.execute(
                """
...
# History queries - all four tables below share the same shape: user_id + jsonb data blob
...
def get_history(user_id: str) -> Dict[str, List[Dict[str, Any]]]:
    # shape returned here is exactly what /api/patient/history sends straight to the frontend
    conn = get_conn()
    history = {
        "predictions": [],
        "scans": [],
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            # scans and food checks are heavier payloads (images/notes) so cap at last 5
